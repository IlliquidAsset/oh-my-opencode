...
import { createMuseAgent, MUSE_PROMPT_METADATA } from "./muse"

describe("createMuseAgent", () => {
  test("returns agent with default model google/gemini-3-pro-preview", () => {
    // #given - no model argument

    // #when
    const agent = createMuseAgent()
  ...
  test("returns agent with custom model when provided", () => {
    ...
    // #when
    const agent = createMuseAgent(customModel)
...
describe("MUSE_PROMPT_METADATA", () => {
  test("has category advisor", () => {
    ...
    // #then
    expect(MUSE_PROMPT_METADATA.category).toBe("advisor")
  })
  ...
  test("has cost CHEAP", () => {
    ...
    // #then
    expect(MUSE_PROMPT_METADATA.cost).toBe("CHEAP")
  })
  ...
  test("has triggers defined as non-empty array", () => {
    ...
    // #then
    expect(Array.isArray(MUSE_PROMPT_METADATA.triggers)).toBe(true)
    expect(MUSE_PROMPT_METADATA.triggers.length).toBeGreaterThan(0)
  })
})

describe("createMuseAgent model-specific config", () => {
  test("with GPT model returns reasoningEffort config", () => {
    // #given
    const gptModel = "openai/gpt-5.2"

    // #when
    const agent = createMuseAgent(gptModel)
  ...
  test("with non-GPT model returns thinking config", () => {
    ...
    // #when
    const agent = createMuseAgent(nonGptModel)

    // #then
    expect(agent.thinking).toEqual({ type: "enabled", budgetTokens: 5000 })
